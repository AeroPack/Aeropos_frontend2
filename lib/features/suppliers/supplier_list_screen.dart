...
class _SupplierListScreenState extends State<SupplierListScreen> {
  final _viewModel = ServiceLocator.instance.supplierViewModel;
  ...
  void _handleSync() async {
    ...
    await _viewModel.syncPendingSuppliers();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<SupplierEntity>>(
        stream: _viewModel.allSuppliers,
  ...
  void _showFormDialog({SupplierEntity? supplier}) {
    showDialog(
      context: context,
      builder: (context) => SupplierFormDialog(
        supplier: supplier,
        onSubmit: ({required name, phone, email, address}) async {
          if (supplier == null) {
            await _viewModel.addSupplier(
              name: name,
              phone: phone,
              email: email,
              address: address,
            ...
            await _viewModel.updateSupplier(updated);
          }
        },
      ),
    );
          ...
          Expanded(
            ...
            child: Icon(
              supplier.syncStatus == 0
                  ? Icons.check_circle
                  : Icons.cloud_upload,
              color: supplier.syncStatus == 0 ? Colors.green : Colors.orange,
              size: 18,
            ),
          ),
